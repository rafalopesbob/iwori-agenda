...
<div class="flex flex-wrap items-center justify-between gap-4 mb-6">
    ...
    <div class="flex items-center gap-2">
        ...
        <a href="{{ route('billing.index') }}"
           class="px-3 py-2 rounded-lg bg-white border border-emerald-200 text-emerald-700 hover:bg-emerald-50 font-medium">Mês atual</a>
...
<div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
    <div class="bg-white rounded-2xl shadow p-5">
        ...
        <p class="text-2xl font-semibold text-emerald-700 mt-1">R$ {{ number_format($totals['total'], 2, ',', '.') }}</p>
    </div>
    <div class="bg-white rounded-2xl shadow p-5">
    ...
    @else
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    @foreach ($rows as $row)
                        <tr class="hover:bg-emerald-50/40">
                            <td class="px-4 py-3">
                                <a href="{{ route('clients.show', $row['client']) }}" class="font-medium text-emerald-800 hover:underline">
                                    {{ $row['client']->name }}
                                </a>
                            </td>
                ...
                <tfoot class="bg-emerald-50 font-semibold text-emerald-900">
                    <tr>
