...
<div class="bg-white rounded-2xl shadow overflow-hidden">
    @if ($clients->isEmpty())
        <p class="p-8 text-gray-500 text-center">
            ...
            <a href="{{ route('clients.create') }}" class="text-mvteal-dark font-medium hover:underline">Cadastre o primeiro</a>.
        </p>
    @else
                ...
                <tbody class="divide-y divide-gray-100">
                    @foreach ($clients as $client)
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3">
                                <a href="{{ route('clients.show', $client) }}" class="font-medium text-mvteal-dark hover:underline">
                                    {{ $client->name }}
                                </a>
                            </td>
                            ...
                            <td class="px-4 py-3">
                                @if ($client->active)
                                    <span class="bg-mvteal-light text-mvteal-dark text-xs font-medium px-2 py-1 rounded-full">Ativo</span>
                                @else
                                    <span class="bg-gray-100 text-gray-600 text-xs font-medium px-2 py-1 rounded-full">Inativo</span>
                                @endif
                            </td>
                            <td class="px-4 py-3 text-right">
                                <a href="{{ route('clients.edit', $client) }}" class="text-mvteal-dark hover:underline font-medium mr-3">Editar</a>
                                <form method="POST" action="{{ route('clients.destroy', $client) }}" class="inline"
                                    ...
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-mvrose-dark hover:underline font-medium">Remover</button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
