...
@section('title', 'Nova sessão — Iwori Agenda')

@section('content')
<div class="max-w-2xl mx-auto bg-white rounded-2xl shadow p-8">
    <h1 class="text-2xl font-semibold text-emerald-800 mb-6">Nova sessão</h1>

    @if ($clients->isEmpty())
        <p class="text-gray-600">
            Você ainda não tem clientes ativos.
            <a href="{{ route('clients.create') }}" class="text-emerald-700 font-medium hover:underline">Cadastre um cliente</a>
        ...
        <form method="POST" action="{{ route('sessions.store') }}" class="space-y-4">
            ...
            <div>
                ...
                <select id="client_id" name="client_id" required
                        class="w-full rounded-lg border-gray-300 border px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500">
            ...
            <div>
                ...
                <textarea id="notes" name="notes" rows="3"
                          class="w-full rounded-lg border-gray-300 border px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500">{{ old('notes') }}</textarea>
