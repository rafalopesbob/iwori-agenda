...
<div class="flex flex-wrap items-center justify-between gap-4 mb-6">
    <h1 class="text-2xl font-semibold text-gray-900">Bem-vindo(a), {{ auth()->user()->name }}!</h1>
    <div class="flex items-center gap-2">
        <a href="{{ route('sessions.create') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg font-medium">Nova sessão</a>
...
<div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
    <a href="{{ route('billing.index') }}" class="bg-white rounded-2xl shadow p-5 hover:shadow-md transition-shadow">
        ...
        <p class="text-2xl font-semibold text-indigo-600 mt-1">R$ {{ number_format($monthTotal, 2, ',', '.') }}</p>
    </a>
    <a href="{{ route('billing.index') }}" class="bg-white rounded-2xl shadow p-5 hover:shadow-md transition-shadow">
...
<div class="bg-white rounded-2xl shadow overflow-hidden">
    <div class="flex items-center justify-between px-6 pt-5 pb-3">
        <h2 class="font-semibold text-gray-900">Sessões de hoje — {{ today()->locale('pt_BR')->translatedFormat('d \d\e F') }}</h2>
        <a href="{{ route('sessions.index') }}" class="text-sm text-indigo-600 hover:underline font-medium">Ver agenda completa</a>
    </div>
                    ...
                    @if ($session->status === App\Enums\SessionStatus::Scheduled)
                        <div class="flex gap-2">
                            <form method="POST" action="{{ route('sessions.status', $session) }}">
                                ...
                                <button type="submit" class="text-xs bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-1.5 rounded-lg font-medium">✓ Realizado</button>
                            </form>
                            <form method="POST" action="{{ route('sessions.status', $session) }}">
                                @csrf
                                @method('PATCH')
